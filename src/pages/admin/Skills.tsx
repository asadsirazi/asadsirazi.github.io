...
const AdminSkills = () => {
  ...
  const save = async (id: string, updates: any) => {
    ...
    else { toast({ title: 'Saved!' }); qc.invalidateQueries({ queryKey: ['skills'] }); }
  };

  const addItem = async () => {
    const maxOrder = items?.length ? Math.max(...items.map(i => i.sort_order || 0)) + 1 : 0;
    const { error } = await supabase.from('skills').insert({ title_en: 'New Category', icon_name: 'Code', skills: [], sort_order: maxOrder });
    if (error) toast({ title: 'Error', description: error.message, variant: 'destructive' });
    else { toast({ title: 'Added!' }); qc.invalidateQueries({ queryKey: ['skills'] }); }
  };

  const deleteItem = async (id: string) => {
    const { error } = await supabase.from('skills').delete().eq('id', id);
    if (error) toast({ title: 'Error', description: error.message, variant: 'destructive' });
    else { toast({ title: 'Deleted!' }); qc.invalidateQueries({ queryKey: ['skills'] }); }
  };
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={addItem}><Plus className="h-4 w-4 mr-2" />Add Category</Button>
      </div>
      {items?.map((item) => (
        <Card key={item.id}>
          ...
          <CardContent className="space-y-4">
            <div><Label>Icon Name</Label>
              <Input defaultValue={item.icon_name || ''} onBlur={e => save(item.id, { icon_name: e.target.value })} /></div>
            <TrilingualInput
              label="Title"
              valueEn={item.title_en} valueBn={item.title_bn || ''} valueAr={item.title_ar || ''}
              onChangeEn={(v) => save(item.id, { title_en: v })}
              onChangeBn={(v) => save(item.id, { title_bn: v })}
              onChangeAr={(v) => save(item.id, { title_ar: v })}
            />
            <div><Label>Skills (JSON array)</Label>
              ...
              }} className="font-mono text-xs" rows={6} /></div>
